    ...
    return (
        <>
            <nav className="fixed top-0 left-0 w-full bg-[#E1E1E1] z-50 shadow-md">
                <div className="container mx-auto flex items-center justify-between h-[80px] px-4 lg:px-8 max-w-screen-xl">
                    {/* Logo / Brand */}
    ...
    {item.name}
                ...
                <div
                    className={`absolute left-0 w-full bg-[#E1E1E1] shadow-md lg:hidden transition-all duration-300 overflow-hidden ${
                        isOpen ? "top-[80px] opacity-100" : "top-[-200px] opacity-0"
                    }`}
                >
                    <ul className="flex flex-col items-center py-4 space-y-4 font-medium">
                        {navigation.map((item) => (
                            <li key={item.name} className="cursor-pointer text-sm">
